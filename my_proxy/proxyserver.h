...
class Proxy{
 ...
 Proxy(std::string port_num):port(port_num),hostname(NULL),proxy_fd(0),client_fd(0),webserver_fd(0){}
  ...
  void revalidate(Request request, Response response_cache,int ID,LRUCache& cache){
    std::ofstream myfile;
  ...
  //cache helper
  void cacheHelper(Request request,int ID,LRUCache& cache){
    std::ofstream myfile;
    //check cache
    ...
    if(control.find("no-cache")!=std::string::npos||control.find("max-age=0")!=std::string::npos){
      ...
      myfile.close();
      revalidate(request,response_cache,ID,cache);
      return;
    }
